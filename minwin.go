...
// Deativated returns the event that will be published when mw is deactivated.
...
func (mw *MinWin) SetFocus(topLevel, focusTo win.HWND) {
	if topLevelWindow := windowFromHandle(topLevel); topLevelWindow != nil {
		if form, ok := topLevelWindow.(Form); ok {
